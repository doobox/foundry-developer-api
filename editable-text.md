...
<p>Unformatted values remain plain text and are HTML-escaped. When formatting is used, Foundry stores the value in a versioned internal form and emits only the supported inline markup. Block developers do not need to change their template.</p>
<h2>Trusted HTML</h2>
...
<p><code>{{ html("content") }}</code> creates a source editor for HTML, CSS and JavaScript. HTML is inserted unescaped at the primitive’s position. Its CSS is emitted with that block instance’s page CSS and its JavaScript with that instance’s page script. Foundry supplies a small list when <code>default:</code> is omitted.</p>
...
```html
<figure class="media {{ block.class }}" {{ block.attributes }}>
    {{ image("hero") }}
</figure>
```
...
<p>Every editable macro requires a quoted developer-defined name. It must begin with a letter and may then contain letters, numbers, underscores and hyphens. Names must be unique across all editable macros and drop zones in the block and cannot duplicate a <code>controls</code> control <code>id</code>.</p>

<div markdown="1">

```html
<article class="article {{ block.class }}" {{ block.attributes }}>
...
<p>The name is part of the block’s persistent API. Reordering named macros leaves their values attached correctly. Removing a macro preserves its stored value, and restoring the same name reconnects it. Changing a name removes the old editable area and creates a new one.</p>
