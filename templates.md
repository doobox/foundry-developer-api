...
<h1>Declare exactly the files your block uses</h1>
...
<p>Foundry does not scan the pack and guess which files are templates. Every HTML, CSS, JavaScript or PHP template the block uses must be declared explicitly in this dictionary.</p>

<div class="note">
<strong>Paths are relative to Contents/Resources.</strong> Every declared file must exist inside the block pack. Foundry determines an additional template’s type from its filename extension.</div>

<h2 id="primary-html">Primary HTML</h2>
<p>Every block needs one primary HTML template. Name it with the <code>html</code> key inside the <code>templates</code> dictionary. Do not also list it in <code>files</code>.</p>
...
<p>Foundry processes this file once for every block placed on a page. Its root element must contain <code>{{ block.attributes }}</code>, which supplies the internal <code>data-foundry-id</code> identity, author-supplied root attributes, and canvas hooks Foundry needs. Its root <code>class</code> attribute must also contain <code>{{ block.class }}</code>, which merges Foundry’s generated classes with classes added in the Advanced Inspector.</p>


<div markdown="1">

```xml
<section class="callout {{ block.class }}" {{ block.attributes }}>
    <h2>{{ text("heading") }}</h2>
</section>
```
...
<strong>The root ID is reserved.</strong> Do not declare <code>id</code> on the block root. Foundry supplies the site author’s optional anchor there. Descendant elements may use developer-owned IDs.</div>
...
<span>File</span><span>Allowed scope</span><span>Optional keys</span><span>Where it goes</span>
    ...
    <div class="api-row">
<strong>Primary HTML</strong><span>Instance, fixed</span><span>None</span><span>Block markup</span>
</div>
    <div class="api-row">
...
<dd>Once for every placed block. This is the default when <code>scope</code> is omitted. Instance-specific values such as <code>{{ instance.id }}</code> and <code>{{ control.name }}</code> are available.</dd>
<dt><code>page</code></dt>
<dd>Once per block pack on each page that uses it, regardless of how many instances appear. Values belonging to one particular instance are unavailable.</dd>
<dt><code>site</code></dt>
<dd>Once per block pack for the entire website. This scope is supported only by CSS.</dd>
</dl>
<div class="callout warning">
<strong>Shared template files cannot use instance values.</strong> Page- and site-scoped files cannot use instance, block, control, child-area, drop-zone, or editable-content macros because no single block instance owns their output. They may use conditional directives with the environment values <code>canvas</code>, <code>preview</code>, and <code>published</code>.</div>
...
<h3>Instance CSS</h3>
<p>Use instance scope when the stylesheet needs a control value or a selector unique to one placed block. Omitting <code>scope</code> means <code>instance</code>.</p>
...
<h3>Page CSS</h3>
<p>Use page scope for styles needed once on every page containing the block pack.</p>
...
```xml
<dict>
    <key>path</key><string>block.js</string>
...
<p><code>closure</code> is optional and defaults to <code>true</code>. Foundry wraps each instance script in a private function receiving <code>block</code>, the block’s root element, and provides a <code>foundry</code> relationship helper in the same private scope. A page-scoped closure receives <code>page</code>, which is the browser document.</p>
...
<p>The instance-scoped <code>foundry</code> helper finds nested block roots without confusing them with ordinary HTML inside the block. Every method may optionally receive a package identifier to filter its result.</p>
<dl class="syntax-list">
<dt><code>foundry.children(packageIdentifier?)</code></dt><dd>Direct child blocks, even when child-area or drop-zone markup places wrappers between their roots.</dd>
<dt><code>foundry.descendants(packageIdentifier?)</code></dt><dd>All nested blocks in document order.</dd>
<dt><code>foundry.parent()</code></dt><dd>The direct parent block, or <code>null</code> for a page-level block.</dd>
<dt><code>foundry.ancestors(packageIdentifier?)</code></dt><dd>All ancestor blocks, nearest first.</dd>
...
<dict>
    ...
    <array>
        <dict>
            <key>path</key><string>block.php</string>
            <key>scope</key><string>instance</string>
        </dict>
    </array>
</dict>
...
<p>The top-level <code>requiresPHP</code> key is optional and defaults to <code>false</code>, but it must be <code>true</code> whenever the pack declares a PHP template or PHP asset. It also forces pages containing the block to use a <code>.php</code> extension. This remains useful when PHP is an independent endpoint called by JavaScript rather than inline page source.</p>
...
<p>The file named by <code>templates</code> → <code>html</code> creates each block instance. Any other <code>.html</code> file is page-level supporting markup and must declare both <code>scope = page</code> and a <code>placement</code>.</p>
...
<dd>Before normal page block markup.</dd>
<dt><code>bodyEnd</code></dt>
<dd>After normal page block markup.</dd>
</dl>
<p>Foundry emits the file once on each page using the block pack. Extra HTML does not support instance or site scope.</p>
...
<p>Foundry processes editor CSS once per block instance, so instance and control macros are available. It can improve empty states, selection, and drop targets in the editing canvas, but it is never included in browser preview or production export.</p>
...
<div class="page-links">
    <a class="card" href="template-identity.html"><strong>Template language</strong><p>Browse identity, controls, assets, conditions, persistent areas, drop zones and block hooks.</p></a>
    <a class="card" href="manifest-identity.html"><strong>Identity and metadata</strong><p>Review the manifest keys that identify and present the block.</p></a>
</div>
{% endraw %}
