---
layout: default
title: Margin control · Foundry Developer
permalink: "/margin-control.html"
---
{% raw %}
...
<h1>Margin</h1>
<p class="lede">A four-sided spacing editor for space outside an element, with theme values, local custom values and Auto.</p>
...
Identifies this item as Margin. Always use `margin`.
...
Exactly four strings in top, right, bottom, left order. Each value must be `none`, `custom`, `auto`, or a portable theme spacing ID: `3xs`, `2xs`, `xs`, `sm`, `md`, `lg`, `xl`, `2xl`, or `3xl`.
...
These keys sit directly in the same custom-item dictionary. Omitted optional keys use the defaults shown. `count` and `units` do not apply because Margin always edits all four sides.
...
Exactly four initial pixel values in top, right, bottom, left order, used when the matching side is set to Custom.
...
Lower bound for custom pixel values; negative margins are allowed.
...
`{{ control.margin }}` resolves as **CSS margin shorthand String**. Stored internally, its value is **One structured spacing value**.
...
<array>
    <dict>
        <key>id</key><string>margin</string>
        <key>label</key><string>Margin</string>
        <key>group</key><string>Content</string>
        <key>type</key><string>margin</string>
        <key>customValue</key>
