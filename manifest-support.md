...
<p>An absolute URL for the part’s documentation or support page. Use a stable HTTPS address that explains the part version users currently have installed.</p>
...
<p>Fallback information Foundry snapshots into each placed part while its package is available. The dictionary accepts optional <code>message</code> and <code>preview</code> strings. <code>preview</code> contains HTML and is used for production output if the package later becomes unavailable.</p>
...
<p>Keep fallback markup small, self-contained, and safe to publish without the rest of the pack. It is recovery information, not an alternative implementation of the part.</p>
<div class="note">
<strong>Current API behavior:</strong> Foundry accepts <code>message</code> as part of the version 1 manifest shape, but does not currently present it in the missing-part interface. Do not rely on it for essential recovery instructions.</div>
</section>
...
<div class="page-links">
    <a class="card" href="manifest-identity.html"><strong>Identity and metadata</strong><p>Review the required part identity and presentation keys.</p></a>
    <a class="card" href="bundle-structure.html"><strong>Development packs</strong><p>Use <code>.foundrydevpack</code> while editing and reloading a part.</p></a>
</div>
{% endraw %}
