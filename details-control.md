---
layout: default
title: Details control · Foundry Developer
permalink: "/details-control.html"
---
{% raw %}
...
<h1>Details</h1>
<p class="lede">A compact labelless Boolean details button.</p>
...
Identifies this item as Details. Always use `details`.
...
<h3>Describing the labelless button</h3>

The control intentionally has no visible row label. Put its explanation in the property-level `toolTip`.
...
> **Important:** Supply toolTip on the property because this control deliberately has no visible label.
</div>

## Return value

`{{ control.advanced }}` resolves as **true or false**. Stored internally, its value is **Boolean**.

```text
{{ if control.advanced }}advanced{{ endif }}
```
...
<array>
    <dict>
        <key>id</key><string>advanced</string>
        <key>label</key><string>Details</string>
        <key>group</key><string>Content</string>
        <key>type</key><string>details</string>
